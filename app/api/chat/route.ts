...
export async function POST(req: NextRequest) {
  try {
    const { messages, useRAG = false, useLiveSources = false } = await req.json()

    if (!messages || messages.length === 0) {
      return new Response("Messages are required", { status: 400 })
    }

    console.log("Chat request received:", { messageCount: messages.length, useRAG, useLiveSources })
    ...
    if (useRAG && lastMessage.role === "user") {
      try {
        ...
        if (ragResponse.ok) {
          ...
          console.log("RAG search successful:", ragData.results?.length || 0, "results")
    ...
    if (useLiveSources && lastMessage.role === "user") {
      try {
        console.log("Attempting live sources search...")
