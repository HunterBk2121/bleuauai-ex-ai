...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await github.listPullRequests(state))
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await github.createPullRequest(prData))
  } catch (error) {
    console.error("GitHub create pull request API error:", error)
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 })
  }
}
