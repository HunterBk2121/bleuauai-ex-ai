...
import { Send, Paperclip, MessageSquare, Clock, Zap, FileText, Scale, BookOpen, RefreshCw } from "lucide-react"
...
export function EnhancedChatInterface() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading, error } = useChat({
    ...
    onResponse: (response) => {
      ...
      // Update connection status and last poll time
      setIsConnected(true)
  ...
  useEffect(() => {
    const pollStatus = async () => {
      try {
        const response = await fetch(`/api/v1/chat/status?sessionId=${sessionId}&t=${Date.now()}`)
        if (response.ok) {
          setIsConnected(true)
          setLastPoll(new Date())
        } else {
    ...
    pollingIntervalRef.current = setInterval(pollStatus, 30000) // Poll every 30 seconds
      ...
      <div className="w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <Button variant="ghost" size="sm" className="text-xs h-6 px-2" onClick={handleRefresh}>
              <RefreshCw className="h-3 w-3 mr-1" />
              Refresh
            </Button>
          </div>
          <div className="text-xs text-gray-500 mt-1">Last updated: {formatLastPoll()}</div>
        </div>
