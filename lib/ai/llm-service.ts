...
export class LLMService {
  ...
  async generateText(request: LLMRequest): Promise<LLMResponse> {
    if (!this.genAI) {
      ...
      try {
        ...
        // Fallback response for diagnostics
        if (request.messages.some((m) => m.content.toLowerCase().includes("diagnostic"))) {
          return {
            text: "OK - LLM service diagnostic response",
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    // For now, return a mock embedding for diagnostics
