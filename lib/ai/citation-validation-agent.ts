...
export class CitationValidationAgent {
  ...
  private async validateSingleCitation(
    ...
    // First, validate with CourtListener
    const courtListenerResult = await this.validateCitationWithCourtListener(citation)

    let bestMatch: any = null
    let highestSimilarity = courtListenerResult.similarity
    let cryptographicHash = ""
    let textualMatch = false

    // If CourtListener found matches, use them as sources
    if (courtListenerResult.clusters.length > 0) {
      bestMatch = {
        id: courtListenerResult.clusters[0].id,
        title: courtListenerResult.clusters[0].caseName,
        content: `${courtListenerResult.clusters[0].caseName} - ${courtListenerResult.clusters[0].court}`,
        court: courtListenerResult.clusters[0].court,
        url: courtListenerResult.clusters[0].url,
        citations: courtListenerResult.clusters[0].citations,
        documentType: "case_law",
        source: "CourtListener",
      }
      textualMatch = true
    } else {
      // Fallback to existing source matching logic
    ...
    return {
      ...
      textualMatch,
      authorityScore,
      validationStatus: courtListenerResult.isValid ? "valid" : validationStatus,
      correctedCitation,
  ...
  /**
   * Validate citation using CourtListener's Citation Lookup API
   */
  private async validateCitationWithCourtListener(citation: { text: string; context: string }): Promise<{
    similarity: number
    isValid: boolean
    clusters: any[]
  }> {
    try {
      const validationResults = await this.courtListener.validateCitations(citation.text)

      if (validationResults.length === 0) {
        return { similarity: 0, isValid: false, clusters: [] }
      }

      const result = validationResults[0]

      // Convert CourtListener status to similarity score
      let similarity = 0
      switch (result.status) {
        case 200: // Found and valid
          similarity = 0.95
          break
        case 300: // Ambiguous but valid
          similarity = 0.85
          break
        case 404: // Valid format but not found
          similarity = 0.3
          break
        case 400: // Invalid format
          similarity = 0.1
          break
        default:
          similarity = 0
      ...
      console.error("CourtListener validation error:", error)
      return { similarity: 0, isValid: false, clusters: [] }
    }
  }
